...
public class Snake implements IEnemy, Observer, Serializable {
	
	//Declare variables
	private String type;
	//private ImageIcon icon = new ImageIcon("snake.png");
	private ImageIcon icon = new ImageIcon(getClass().getClassLoader().getResource("snake.png"));
	...
	//move method for the enemy to move in a neighbouring Square in the Grid
	@Override
	public void move(Grid theGrid) {
		...
		//using recursion to make sure that the snake does not go out of the Grid or remains in the same position
		if(testRow < 1 || testRow > GamePlay.gridSize || testCol < 1 || testCol > GamePlay.gridSize || (testRow == row && testCol == col)){
			...
			removeEnemy(theGrid);								//remove the snake from its current position before moving/adding it to the new position
			
			for ( Row tempRow : theGrid.theGrid) {
				for (Square s : tempRow.theRow) {
					if(s.getNumber() == col && tempRow.getNumber() == row) {
						s.addEnemy(this);						//add the snake to the random position
					}
				}
			}
		}
	...
	//removeEnemy method to remove this snake from the Square
	@Override
	...
	//getRowPosition method to get the current row position of the snake
